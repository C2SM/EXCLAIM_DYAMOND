...
EXP_ROOT="/capstor/scratch/cscs/leclairm/DYAMOND_experiments"
MEMBERS="1"
NODES="2"
IO_PROCS="1"
...
benchmark_with_io(){
    ...
    export N_NODES="${2}"
    export N_IO_PROCS="${3}"
    export EXPDIR="${EXP_ROOT}/Benchmark_R02B06/with_IO/member_${1}/Benchmark_${2}_${3}"
    mkdir -p "${EXPDIR}"
    ./DYAMOND_R02B06L120.run
}

benchmark_no_io(){
    export ENABLE_IO="false"
    export N_NODES="${2}"
    export EXPDIR="${EXP_ROOT}/Benchmark_R02B06/no_IO/member_${1}/Benchmark_${2}"
    mkdir -p "${EXPDIR}"
    ./DYAMOND_R02B06L120.run
}

export basedir="/capstor/store/cscs/userlab/cwd01/leclairm/archive_build_liskov/icon_25.2_v3/icon-exclaim/build_dsl"
export end_date=${end_date:-"2020-01-22T00:00:00"}
